function Msignal= ...
    flipmagnezationCPUParforEchoT2prep(T2prime,preptime,Atd,Btd,Ate,Bte,Atr1,Btr1,Atr2,Btr2,Atr3,Btr3, M0, dephasefactor1, dephasefactor2)
...
%  M_td0(3,:,dictindex)=M0(3,:).*exp(-preptime./T2prime(dictindex));   code
%  from previous version of preparation

M00=M0(:)*(exp(-preptime./T2prime))';   % Yifan modified 8/7/2019
M0temp=double(M00(:));
 
% M0temp=double(repmat(M0(:), [dictlength,1]));   % no T2 preparation
...
for expindex=1:expnumber
    ...
    clear atd btd atdmatrix btdmatrix
